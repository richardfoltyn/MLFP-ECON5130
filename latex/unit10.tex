\documentclass{scrartcl}
    ...
    % Document parameters
    % Document title
    \newcommand*{\mytitle}{Unit 10: Introduction to supervised learning}
...
\] where \(L\) is the loss function that depends on the choice
parameters. Note that we use the ``hat'' notation \(\widehat{\alpha}\)
to distinguish the OLS estimate from the (usually unknown) true
parameter \(\alpha\). The exact value of \(\widehat{\alpha}\) will vary
based on the estimator used as we will see later in this unit.
...
where we have a vector of explanatory variables \(\mathbf{x}_i\) (which
...
For econometrics and machine learning, it usually makes sense to use
linear regression models such as those implemented in
...
\end{tcolorbox}
    ...
    The following code visualised the sample as a scatter plot of
...
in the \texttt{coef\_} attribute (which is an array even if there is a
single explanatory variable). If the model includes an intercept, its
fitted value is stored in the attribute \texttt{intercept\_}.
...
synthetic sample, but in general unknown for real data. For small sample
sizes, the true and estimated parameters need not be close, as the graph
illustrates. The graph also shows the prediction errors as the dotted
lines between the sample points \(y_i\) and the black fitted model. OLS
minimizes the sum of the squares of these distances.
...
predict outcomes for new data which has not been used for estimation.
    ...
    To demonstrate the use of training and test data sets, we will use as
...
data sets suitable for machine learning tasks.

The original data set has 80 features (explanatory variables) which are
house characteristics of houses in Ames, a city of about 60 thousand
inhabitants in the middle of Iowa, USA. The goal is to use these
features to predict the house prices (or ``target'' in ML terminology).
The above website provides details on all 80 features, but we will
restrict ourselves to a small subset.
...
memory usage: 148.4+ KB
    ...
    In this section, we focus on the columns \texttt{SalePice} which
...
remaining to the training data set. Once we have randomly allocated 90\%
...
\PY{c+c1}{\PYZsh{} Randomly assign observations to training or test samples}
...
\PY{c+c1}{\PYZsh{} Test sample is complement to training sample}
...
\PY{n}{ax}\PY{o}{.}\PY{n}{scatter}\PY{p}{(}\PY{n}{X\PYZus{}train}\PY{p}{,} \PY{n}{y\PYZus{}train}\PY{p}{,} \PY{n}{s}\PY{o}{=}\PY{l+m+mi}{15}\PY{p}{,} \PY{n}{color}\PY{o}{=}\PY{l+s+s1}{\PYZsq{}}\PY{l+s+s1}{none}\PY{l+s+s1}{\PYZsq{}}\PY{p}{,} \PY{n}{edgecolor}\PY{o}{=}\PY{l+s+s1}{\PYZsq{}}\PY{l+s+s1}{steelblue}\PY{l+s+s1}{\PYZsq{}}\PY{p}{,} 
    \PY{n}{alpha}\PY{o}{=}\PY{l+m+mf}{1.0}\PY{p}{,} \PY{n}{lw}\PY{o}{=}\PY{l+m+mf}{0.5}\PY{p}{,} \PY{n}{label}\PY{o}{=}\PY{l+s+sa}{r}\PY{l+s+s1}{\PYZsq{}}\PY{l+s+s1}{\PYZdl{}y\PYZus{}}\PY{l+s+si}{\PYZob{}train\PYZcb{}}\PY{l+s+s1}{\PYZdl{}}\PY{l+s+s1}{\PYZsq{}}\PY{p}{)}
...
\begin{Verbatim}[commandchars=\\\{\}]
<matplotlib.legend.Legend at 0x7fa67c3369b0>
\end{Verbatim}
\end{tcolorbox}
...
\begin{Verbatim}[commandchars=\\\{\}]
<matplotlib.lines.Line2D at 0x7fa67c208a00>
\end{Verbatim}
\end{tcolorbox}
...
is missing some explanatory variables.
...
test sample, the \(R^2\) could even be negative.
    ...
    While these measures are easily to implement ourselves, we can just as
...
Coefficient of determination (R\^{}2): 0.58
    ...
    Since we estimated a model with intercept, the \(R^2 = 0.47\) implies
that the model explains 47\% of the variance in the test sample.
...
the Ames housing data set again but use several (continuous) explanatory
variables.
...
\(m^2\)) as a feature to the model, which is therefore given by \[
SalePrice_i = \alpha + \beta_0 LivingArea_i + \beta_1 LotArea_i + \epsilon_i
\]

As usually, we inspect the descriptive statistics of the newly added
...
values of the remaining features at some value or use 3D scatter plots.
The latter of course does not help if we have more than two explanatory
variables. Instead, we can plot the prediction errors against \(y\)
which we do below.
...
\end{tcolorbox}
    ...
    \begin{center}
    \adjustimage{max size={0.9\linewidth}{0.9\paperheight}}{unit10_files/unit10_41_1.pdf}
    \end{center}
...
transformations of explanatory and response variables.
    ...
    To illustrate, we now extend the previous model which included
...
Coefficients [-2.19724359e+04  1.24027918e+03  5.75108671e+01  3.95841742e-01
 -2.46346339e-01 -3.77428502e-04]
    ...
    The fitted model now has 6 coefficients and the intercept is 0 as the
...
response variable. However, it is not important to also transform the
original explanatory variables in the test data set using the same
...
\PY{n}{y\PYZus{}test\PYZus{}hat} \PY{o}{=} \PY{n}{lr}\PY{o}{.}\PY{n}{predict}\PY{p}{(}\PY{n}{poly}\PY{o}{.}\PY{n}{transform}\PY{p}{(}\PY{n}{X\PYZus{}test}\PY{p}{)}\PY{p}{)}
...
\end{tcolorbox}
    ...
    \begin{center}
    \adjustimage{max size={0.9\linewidth}{0.9\paperheight}}{unit10_files/unit10_49_1.pdf}
    \end{center}
...
an (arbitrary) name and the class that will be created for that step. To
create a pipeline that creates polynomial features and fits a linear
model to them, we would therefore proceed as follows:
...
step). These names are occasionally required to retrieve information
from a specific step. To demonstrate that the pipeline generates results
that are equivalent to our manual implementation, we conclude this
section by recreating the error plot from above.
...
\end{tcolorbox}
    ...
    \begin{center}
    \adjustimage{max size={0.9\linewidth}{0.9\paperheight}}{unit10_files/unit10_57_1.pdf}
    \end{center}
...
\end{figure}
    ...
    Note that in this unit we will skip the final step of assessing
generalisability on the test data set. Consequently, when running
cross-validation, we will only use training/validation data sets and we
will use the terms ``validation'' and ``test'' interchangeably.
...
To illustrate the concept, we will demonstrate the procedure outlined
above by tuning the polynomial degree for the example covered in the
previous section.
...
\PY{k}{for} \PY{n}{d} \PY{o+ow}{in} \PY{n}{degrees}\PY{p}{:}
    ...
    \PY{n}{mse\PYZus{}fold} \PY{o}{=} \PY{p}{[}\PY{p}{]}
    ...
    \PY{k}{for} \PY{n}{itrain}\PY{p}{,} \PY{n}{itest} \PY{o+ow}{in} \PY{n}{folds}\PY{o}{.}\PY{n}{split}\PY{p}{(}\PY{n}{X}\PY{p}{,} \PY{n}{y}\PY{p}{)}\PY{p}{:}
        ...
        \PY{c+c1}{\PYZsh{} Compute MSE as model fit measure}
        \PY{n}{mse} \PY{o}{=} \PY{n}{mean\PYZus{}squared\PYZus{}error}\PY{p}{(}\PY{n}{y\PYZus{}test}\PY{p}{,} \PY{n}{y\PYZus{}test\PYZus{}hat}\PY{p}{,} \PY{n}{squared}\PY{o}{=}\PY{k+kc}{False}\PY{p}{)}

        \PY{c+c1}{\PYZsh{} Store MSE for current split}
        \PY{n}{mse\PYZus{}fold}\PY{o}{.}\PY{n}{append}\PY{p}{(}\PY{n}{mse}\PY{p}{)}

    \PY{c+c1}{\PYZsh{} Store average MSE for current degree}
    \PY{n}{mse\PYZus{}mean}\PY{o}{.}\PY{n}{append}\PY{p}{(}\PY{n}{np}\PY{o}{.}\PY{n}{mean}\PY{p}{(}\PY{n}{mse\PYZus{}fold}\PY{p}{)}\PY{p}{)}
...
\end{tcolorbox}
    ...
    This code returns an array of 5 averaged MSEs, one for each
...
Polynomial degree with min. RMSE: 2
    ...
    Finally, it is often instructive how the MSE evolves as a function of
...
\begin{Verbatim}[commandchars=\\\{\}]
<matplotlib.lines.Line2D at 0x7fa6503307c0>
\end{Verbatim}
\end{tcolorbox}
        
    \begin{center}
    \adjustimage{max size={0.9\linewidth}{0.9\paperheight}}{unit10_files/unit10_68_1.pdf}
    \end{center}
...
test sample and a very large MSE.
...
sample splitting for us. However, \texttt{scikit-learn} provides us with
even more convenience functions that further simplify this process. For
example, if we want to perform tuning based on a single score (such as
the mean squared error), we can instead use
...
metric to evaluate. For example, if we want to compute root mean squared
error (RMSE), we would pass the argument
...
for a completely list of valid metrics that can be passed to
\texttt{scoring=...}. Alternatively, \texttt{scikit-learn} lists
available metrics by running

\begin{Shaded}
\begin{Highlighting}[]
\ImportTok{import}\NormalTok{ sklearn.metrics}
\NormalTok{sklearn.metrics.get_scorer_names()}
\end{Highlighting}
\end{Shaded}
...
\PY{k}{for} \PY{n}{d} \PY{o+ow}{in} \PY{n}{degrees}\PY{p}{:}
    ...
    \PY{n}{mse\PYZus{}mean}\PY{o}{.}\PY{n}{append}\PY{p}{(}\PY{n}{np}\PY{o}{.}\PY{n}{mean}\PY{p}{(}\PY{o}{\PYZhy{}}\PY{n}{score}\PY{p}{)}\PY{p}{)}
    \PY{c+c1}{\PYZsh{} Compute std. dev. of RMSE across splits}
    \PY{n}{mse\PYZus{}std}\PY{o}{.}\PY{n}{append}\PY{p}{(}\PY{n}{np}\PY{o}{.}\PY{n}{std}\PY{p}{(}\PY{o}{\PYZhy{}}\PY{n}{score}\PY{p}{)}\PY{p}{)}
...
\end{tcolorbox}
    ...
    There are several ways to deal with such data. First, since the data is
stored as strings, we could use
...
Using a sparse matrix consequently saves memory, but makes interacting
with the resulting array more cumbersome. For small data sets, there is
no need to use sparse matrices.
...
which accomplishes something very similar to \texttt{OneHotEnconder},
but may not work as well with the \texttt{scikit-learn} API.
...
In applied work, we frequently encounter interactions of explanatory
variables, \ie an explanatory variable is formed as the product of two
other variable. For example, if our data contains the variables \(x\)
and \(z\), we might add \(x \cdot z\) as an independent variable to the
model.
...
\tightlist
\item
  Create a sample of size \(N=50\) where the \(x_i\) are randomly draw
...
the model without and additional intercept as the intercept is already
...
These allow us to fuse several transformation and ultimate estimation
steps into a single model.

Continuing with the setup in the previous exercise, we now focus on the
model with polynomial degree \(d=5\). Use either
\href{https://scikit-learn.org/stable/modules/generated/sklearn.pipeline.make_pipeline.html}{\texttt{make\_pipeline()}}
or the
...
\href{https://scikit-learn.org/stable/modules/generated/sklearn.preprocessing.PolynomialFeatures.html}{\texttt{PolynomialFeatures}}
transformation and fitting a linear model in a single step.
...
y_i = \sin\bigl(2 \pi (x + 0.1) \bigr) + \epsilon_i
...
\item
  ...
  \emph{Hint:} The \(\sin(\bullet)\) function and the constant \(\pi\)
  are part of NumPy and can be accessed as \texttt{np.sin()} and
  \texttt{np.pi}.
\item
...
\end{tcolorbox}
    ...
    It is again convenient to use a \texttt{DataFrame} to print a few
observations of the interaction terms that were created.
...
\end{tcolorbox}
    ...
    \begin{center}
    \adjustimage{max size={0.9\linewidth}{0.9\paperheight}}{unit10_files/unit10_115_1.pdf}
    \end{center}
...
\end{tcolorbox}
    ...
    \begin{center}
    \adjustimage{max size={0.9\linewidth}{0.9\paperheight}}{unit10_files/unit10_119_1.pdf}
    \end{center}
...
\end{tcolorbox}
    ...
    \begin{center}
    \adjustimage{max size={0.9\linewidth}{0.9\paperheight}}{unit10_files/unit10_123_0.pdf}
    \end{center}
...
\end{tcolorbox}
    ...
    \begin{center}
    \adjustimage{max size={0.9\linewidth}{0.9\paperheight}}{unit10_files/unit10_126_1.pdf}
    \end{center}
...
\end{tcolorbox}
    ...
    \begin{center}
    \adjustimage{max size={0.9\linewidth}{0.9\paperheight}}{unit10_files/unit10_128_0.pdf}
    \end{center}
    
    As the figure shows, degree-0 polynomial is just a constant, while a
...
\begin{Verbatim}[commandchars=\\\{\}]
<matplotlib.legend.Legend at 0x7fa65d549390>
\end{Verbatim}
\end{tcolorbox}
        
    \begin{center}
    \adjustimage{max size={0.9\linewidth}{0.9\paperheight}}{unit10_files/unit10_137_1.pdf}
    \end{center}
...
\end{tcolorbox}
    ...
    \begin{center}
    \adjustimage{max size={0.9\linewidth}{0.9\paperheight}}{unit10_files/unit10_140_1.pdf}
    \end{center}
...
\item
  ...
  each split and computes the MSE on test (or validation) sample.
\end{enumerate}
...
function) on your sample data. The functions returns 10 different MSE
values, one for each test sample of each split.
...
\begin{Verbatim}[commandchars=\\\{\}]
<matplotlib.lines.Line2D at 0x7fa6486194b0>
\end{Verbatim}
\end{tcolorbox}
        
    \begin{center}
    \adjustimage{max size={0.9\linewidth}{0.9\paperheight}}{unit10_files/unit10_151_1.pdf}
    \end{center}
...
\begin{Verbatim}[commandchars=\\\{\}]
<matplotlib.legend.Legend at 0x7fa648570b80>
\end{Verbatim}
\end{tcolorbox}
        
    \begin{center}
    \adjustimage{max size={0.9\linewidth}{0.9\paperheight}}{unit10_files/unit10_153_1.pdf}
    \end{center}
...
\begin{Verbatim}[commandchars=\\\{\}]
<matplotlib.lines.Line2D at 0x7fa648433970>
\end{Verbatim}
\end{tcolorbox}
        
    \begin{center}
    \adjustimage{max size={0.9\linewidth}{0.9\paperheight}}{unit10_files/unit10_160_1.pdf}
    \end{center}
...
\begin{Verbatim}[commandchars=\\\{\}]
<matplotlib.lines.Line2D at 0x7fa6485e88b0>
\end{Verbatim}
\end{tcolorbox}
        
    \begin{center}
    \adjustimage{max size={0.9\linewidth}{0.9\paperheight}}{unit10_files/unit10_167_1.pdf}
    \end{center}
